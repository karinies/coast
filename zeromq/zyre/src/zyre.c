...
//  ---------------------------------------------------------------------
//  Sends message comprising two frames to single peer, specified as a UUID string
//  Deallocating the frame contents is the responsibilty of the caller.
//  --- Created for the sake of Motile/Island 2014/05/05 ---
int
zyre_island_whisper (zyre_t *self, char *peer, const void *frame1, size_t size1, const void *frame2, size_t size2)
{
  assert (self);
  assert (peer);
  assert (frame1);
  ...
  // printf("inside zyre_island_whisper: %s\n", peer); // MMG
  zmsg_pushmem(msg, frame1, size1);
  zmsg_pushmem(msg, frame2, size2);
  // printf("pushed frames onto message\n"); // MMG
  zstr_sendm (self->pipe, "WHISPER");
  // printf("send WHISPER down pipe\n"); // MMG
  zstr_sendm (self->pipe, peer);
  // printf("send peer id down pipe\n"); // MMG
  rc = zmsg_send(&msg, self->pipe);
  // printf("send msg down pipe\n"); // MMG
  return rc;
}
