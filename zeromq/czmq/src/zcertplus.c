...
struct _zcert_t {
  ...
  byte public_sign_key [32];  // Public signing key in binary
  byte secret_sign_Key [64];  // Secret signing key in binary
  char public_sign_txt [41];  // Public signing key in Z85 text
  char secret_sign_txt [81];  // Secret signing key in Z85 text
...
zcert_new (void)
{
    ...
    byte public_sign_key [32] = { 0 };
    byte secret_sign_key [64] = { 0 };
    int rc = crypto_box_keypair (public_key, secret_key);
    assert (rc == 0);
    rc = crypto_sign_kepair (public_sign_key, secret_sign_key);
    assert (rc == 0);
    return zcert_new_from (public_key, secret_key, public_sign_key, secret_sign_key);
#else
    return NULL;
#endif
}
...
//  --------------------------------------------------------------------------
//  Constructor, accepts public/secret key pair and public/secret signing key pair from caller

zcert_t *
zcert_new_from (byte *public_key, byte *secret_key, byte *public_sign_key, byte *secret_sign_key)
{
    ...
    assert (public_sign_key);
    memcpy (self->public_sign_key, public_sign_key, 32);
    zmq_z85_encode (self->public_sign_txt, self->public_sign_key, 32);

    assert (secret_sign_key);
    memcpy (self->secret_sign_key, secret_sign_key, 64);
    zmq_z85_encode (self->secret_sign_txt, self->secret_sign_key, 64);
...
zcert_load (char *format, ...)
{
	...
	char *public_sign_text = zconfig_resolve (root, "/curve/public-sign-key", NULL);
	char *secret_sign_text = zconfig_resolve (root, "/curve/secret-sign-key", NULL);
	    ...
	    if (public_sign_text && strlen (public_sign_text) == 40)
	      zmq_z85_decode (public_sign_key, public_sign_text);
	    if (secret_sign_text && strlen (secret_sign_text) == 80)
	      zmq_z85_decode (secret_sign_key, secret_sign_text;

            //  Load metadata into certificate
	    self = zcert_new_from (public_key, secret_key, public_sign_key, secret_sign_key);
