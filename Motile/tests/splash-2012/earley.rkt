        ...
        (define (conf-set-adjoin** states states* state-num conf i)
          (let ((state (vector-ref states state-num)))
            (if (conf-set-member? state conf i)
              (let* ((state* (vector-ref states* state-num))
                     (conf-set* (conf-set-get* state* state-num conf)))
                (when (not (conf-set-next conf-set* i))
                  (conf-set-adjoin state* conf-set* conf i))
                #t)
              #f)))
...
(define (racket-version)
  (test))
(define BASELINE+VECTORS
  (++ BASELINE (global-defines vector make-vector vector-set! vector-ref
                               list->vector vector-length)))
(define (motile-version)
  (motile/call motile-compiled-source BASELINE+VECTORS))
