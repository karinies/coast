...
;; Copied from Motile/baseline.rkt on 2014-05-18.
;; Simplified from original Motile/baseline.rkt
;; * Deleted ENVIRON/TEST
...
(define BASELINE
  (pairs-to-environ
   ...
   (list
    ...
    (define/global/2 'box/set  set-box!)
    ...
    ; Murmur primitives
    (define/global/1 'murmur? murmur?)
    (define/global/1 'murmur/origin murmur/origin)
    (define/global/1 'murmur/target murmur/target)
    (define/global/1 'murmur/payload murmur/payload)
    
    ; CURL primitives.
    (define/global/1 'curl? curl?)
    (define/global/1 'curl/embargo? curl/embargo?)
    (define/global/1 'curl/origin curl/origin)
    (define/global/1 'curl/metadata curl/metadata)
    (define/global/1 'curl/path curl/path)
    (define/global/1 'curl/verified? curl/verified?)
    )))

;; Still incomplete: time, date, gates.
(define BASELINE/SPAWN
  (pairs-to-environ
   BASELINE
   (list
    ; Hack for now.
    ...
    ; Time and dates.
    (define/global/0 'time/now time/now)
    (define/global/0 'date/now date/now)
    (define/global/0 'time/now/milliseconds time/now/milliseconds)
    ; Date accessors.
    (define/global/1 'date/year        date/year)
    (define/global/1 'date/month       date/month)
    (define/global/1 'date/day         date/day)
    (define/global/1 'date/hour        date/hour)
    (define/global/1 'date/minute      date/minute)
    (define/global/1 'date/second      date/second)
    (define/global/1 'date/millisecond date/millisecond)
    ; Durations.
    (define/global/N 'duration/new     duration/new)
    (define/global/1 'duration?        duration?)
    (define/global/1 'duration-to-milliseconds duration-to-milliseconds)
    
    ; Temporal arithmetic
    (define/global/2 'time- time-)
    (define/global/2 'time+ time+)
    
    ; Promises.
    (define/global/1 'promise?          promise?)
    (define/global/1 'promise/block     promise/block)
    (define/global/1 'promise/new       promise/new)
    (define/global/1 'promise/resolved? promise/resolved?)
    (define/global/1 'promise/resolver  promise/resolver)
    (define/global/1 'promise/try       promise/try)
    (define/global/2 'promise/wait      promise/wait)
    
    ; Duplets (of which promises are a special case).
    (define/global/1 'duplet?         duplet?)
    (define/global/1 'duplet/block    duplet/block)
    (define/global/1 'duplet/resolver duplet/resolver)
    (define/global/1 'duplet/try      duplet/try)
    (define/global/2 'duplet/wait     duplet/wait)
    (define/global/N 'sync            sync)
    (define/global/N 'sync/timeout    sync/timeout)
    
    ; Limited constructor for duplets.
    (define/global/N 'islet/curl/new islet/curl/new)
    
    ; Gates.
    (define/global/1 'gate? gate)
    (define/global/N 'gate/and  gate/and)
    (define/global/1 'gate/not  gate/not)
    (define/global/0 'gate/once gate/once)
    (define/global/N 'gate/or   gate/or)
    (define/global/1 'gate/uses gate/uses)
    (define/global/N 'gate/xor  gate/xor)
    (define/global/1 'gate/blacklist/island gate/blacklist/island)
    (define/global/1 'gate/whitelist/island gate/whitelist/island)
    
    ; Transmission.
    (define/global/2 'send send/strong)
    
    ; UUID
    (define/global/0 'uuid/symbol uuid/symbol)
    (define/global/0 'uuid/bytes  uuid/bytes)
    (define/global/0 'uuid/string uuid/string)
    
    ; Spawns.
    (define/global/2 'spawn/exchange/duplet* spawn/exchange/duplet*)
    (define/global/2 'spawn/control/duplet*  spawn/control/duplet*)
    (define/global/N 'spawn/exchange/curl    spawn/exchange/curl)
