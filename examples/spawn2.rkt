          ...
          (let loop ([m (duplet/block d)])
            (let ([payload (murmur/payload m)])
              (when (and (list? payload) (= (length payload) 3))
                (let ([challenge (car   payload)]  ; Random challenge number
                      [u         (cadr  payload)]  ; Reply CURL
                      [content   (caddr payload)]) ; Content for echo.
                  (when (curl? u)
                    (send u (cons challenge content)))))) ; Echo back to the client.
         ...
         (let* ([p (promise/new)] ; So spawn can reach back to the client.
           ...
           (let* ([m (promise/block p)] [server/echo/u (murmur/payload m)])
             ...
             (let* ([challenge (gate/challenge)]
                    [reply (islet/curl/new (list 'reply spawn/name) challenge #f 'INTER)])
               ...
               (for ([i (in-range 1 10)] #:when (odd? i)) ; Send some odd numbers to the echo server.
                 (send
                  server/echo/u
                  (list
                   (gate:challenge/peek challenge) ; Random challenge number.
                   (duplet/resolver reply)         ; CURL
                   i))                             ; Content to echo.
