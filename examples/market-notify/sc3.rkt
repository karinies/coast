  ...
  (island/compile
   '(lambda (motile/register/market motile/register/risk trader/notif/curl)
      (lambda ()
        ...
        (let ([backdoor (islet/curl/new '(backdoor) GATE/ALWAYS #f 'INTER)])
          (islet/log/info "Sending CURL for backdoor")
          (send trader/notif/curl (duplet/resolver backdoor))
          (islet/log/info "Sent CURL for backdoor")
          (let ([m (duplet/block backdoor)])
            (let ([payload (murmur/payload m)])
              (islet/log/info "BACKDOOR EXPLOITED!"))))
  ...
  (let* ([pr (subislet/callback/new 'trader-notif EXAMPLES/ENVIRON ; create a new islet to listen notifications of order requests made on traders behalf
                                    (island/compile '(lambda (payload) ;callback function  to handle order notifications to the trader
                                                       (if (string? payload)
                                                           ...
                                                           ;(lambda ()
                                                             (send payload "Exploiting backdoor!")
                                                             ;(islet/log/info "SENDING DATA TO BACKDOOR!"))
                                                       ))))]
