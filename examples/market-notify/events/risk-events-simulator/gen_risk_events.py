...
def init_stock_data(infile):
    ...
    return event_count, min_delay, max_delay, stock_risks

def generate_stock_events(outfile, event_count, min_delay, max_delay, stocks):
    for i in range(event_count):
        ...
        print("%s %s %d" % (symbol, "risk", new_risk), file=outfile)    

def main():
    in_filename = "risk_input.txt"
    infile = open(in_filename, "r")
    event_count, min_delay, max_delay, stock_risks = init_stock_data(infile)
    ...
    generate_stock_events(outfile, event_count, min_delay, max_delay, stock_risks)
    infile.close()
    outfile.close()
