...
(struct trie (map slots))
(define-syntax-rule (trie/map t)   (trie-map t))
(define-syntax-rule (trie/slots t) (trie-slots t))
;; Return a skeleton trie with the given map and n slots (one slot for each 1-bit appearing in the map).
(define (trie/construct m n)
  (trie n (make-vector n #f)))

(define-syntax-rule (trie/slot t i)
  (vector-ref (trie/slots t) i))
(define-syntax-rule (trie/slot! t i x)
  (vector-set! (trie/slots t) i x))
(define-syntax-rule (trie/length t) (vector-length (trie/slots t)))
...
;; Given a 32-bit hash code and a shift = 0, 5, 10, ..., 30 return the 32-bit mask = 1 << n
;; where n = 0, 1, ..., 31.
...
(define-syntax-rule (trie/slots/before t mask)
  (bitwise-bit-count (&& (trie/map t) (sub1 mask))))
